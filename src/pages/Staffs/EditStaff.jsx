...
export default function StaffEdit() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // ✅ Corrected: Removed _method="PUT" because the endpoint is now a true POST
        ...
        <div className="mb-6 flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">Update staff details and documents.</p>
            </div>
          </div>
        ...
        <div className="rounded-2xl bg-white p-6 md:p-8 shadow-sm ring-1 ring-slate-100">
          <form className="space-y-8" onSubmit={handleSubmit} noValidate>
            ...
            <section className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div data-field="name">
                  ...
                  <input type="text" className={inputClass("name")} value={name} onChange={(e) => setName(e.target.value)} onBlur={() => markTouched("name")} required />
                ...
                <div data-field="profile_pic">
                  ...
                  <div className="mt-1 flex items-center gap-4">
                    <div className="relative h-12 w-12 shrink-0 overflow-hidden rounded-full border border-slate-200 bg-slate-50">
                        {avatarPreview ? <img src={avatarPreview} alt="Preview" className="h-full w-full object-cover" /> : <div className="flex h-full w-full items-center justify-center text-slate-400">?</div>}
                    </div>
                    <input ref={photoRef} type="file" accept="image/*" className="block w-full text-xs text-slate-500" onChange={handleAvatarChange} />
                  </div>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div data-field="email">
                  ...
                  <input type="email" className={inputClass("email")} value={email} onChange={(e) => setEmail(e.target.value)} onBlur={() => markTouched("email")} required />
                ...
                <div data-field="password">
                  ...
                  <div className="mt-1 relative">
                    <input type={showPwd ? "text" : "password"} placeholder="Leave empty to keep current" className={`${inputClass("password")} pr-10`} value={password} onChange={(e) => setPassword(e.target.value)} />
                  ...
                  <div className="mt-1 flex gap-2">
                    <div className="w-32 shrink-0">
                      <select className={inputClass("phoneCodeId")} value={phoneCodeId} onChange={(e) => {
                        ...
                        {phoneCodes.map((c) => <option key={getId(c)} value={String(getId(c))}>{extractDial(c)}</option>)}
                      </select>
                    </div>
            ...
            <section className="space-y-4 pt-1 border-t border-slate-100">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div data-field="newDocs">
                  ...
                  <input ref={docRef} type="file" multiple className="mt-1 block w-full text-xs text-slate-500" />
                </div>
              </div>
                  ...
                  <div className="mt-2 text-sm">
                      ...
                      <ul className="list-disc pl-5 mt-1">
                          {existingDocs.map((doc, i) => (
                              <li key={doc.id}><a href={doc.url} target="_blank" rel="noreferrer" className="text-indigo-600 hover:underline">{doc.name}</a></li>
                          ))}
                      </ul>
                  </div>
              )}
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
              <button type="button" className="rounded-xl border border-slate-200 bg-slate-100 px-4 py-2 text-xs font-medium text-slate-700" onClick={() => navigate(-1)}>Cancel</button>
              <button type="submit" disabled={submitting} className="rounded-xl bg-indigo-600 px-5 py-2 text-xs font-medium text-white shadow-md hover:bg-indigo-700">{submitting ? "Saving..." : "Save Changes"}</button>
            </div>
          </form>
        </div>
      </div>
