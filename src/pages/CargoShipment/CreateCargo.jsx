...
export default function CreateCargo() {
  ...
  const showToast = useCallback(
    (text, variant = "success", duration = 3500) => {
      try {
    ...
    try {
      ...
      const [
        ...
        cachedMethods
          ...
          : getActiveShipmentMethods().catch(
              (e) => (console.warn("Methods failed:", e), [])
            ),
        cachedStatuses
          ? Promise.resolve(cachedStatuses)
          : getActiveShipmentStatuses().catch(
      ...
      const [branchRes, nextInvoiceRes, staffRes] = await Promise.all([
        preferredBranchId
          ? viewBranch(preferredBranchId).catch(() => null)
          : null,
        preferredBranchId
          ? getNextInvoiceNo(preferredBranchId).catch(() => null)
          : null,
        preferredBranchId
          ? getBranchUsers(preferredBranchId).catch(() => [])
          : [],
      ]);
  ...
  const softResetForNext = useCallback(
    (branchId, nextInvoiceNo) => {
  ...
  const submit = useCallback(
    async (e) => {
      ...
      if (missing.length) {
        setMsg({
          text: `Missing/invalid: ${missing.join(", ")}`,
          variant: "error",
        });
        return;
      }
      ...
      if (!Number.isFinite(roleId) || roleId <= 0) {
        setMsg({
          text: "Choose a valid ‘Collected By’ role.",
          variant: "error",
        });
        return;
      }
      if (!Number.isFinite(personId) || personId <= 0) {
        setMsg({
          text: "Choose a valid ‘Collected By’ person.",
          variant: "error",
        });
        return;
      }
      ...
      try {
        ...
        // Fetch latest booking number again before submission to avoid conflicts
        const freshBookingNo = await getNextInvoiceNo(form.branchId).catch(() => null);
        
        // FIX: Ensure the fresh number matches the current branch logic
        if (freshBookingNo) {
          let validatedBookingNo = freshBookingNo;
          
          // If we have a branch code and the API returned a number (raw or prefixed)
          // We assume we should stick to the branch's prefix format "RUH:xxxxxx"
          const currentBranchCode = form.invoiceNo.split(':')[0];
          if (currentBranchCode && currentBranchCode.length === 3) {
             const numPart = getNumericPart(freshBookingNo);
             // Reconstruct to ensure we don't accidentally take "OTHERBRANCH:123"
             validatedBookingNo = `${currentBranchCode}:${String(numPart).padStart(6, "0")}`;
          }

          updateForm((draft) => {
            draft.invoiceNo = validatedBookingNo;
          });
