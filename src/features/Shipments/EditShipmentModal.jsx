...
export default function EditShipmentModal({ shipmentId, isOpen, onClose, onSuccess }) {
 ...
 const handleSave = async () => {
    setSaving(true);
    try {
      // 1. Create a CLEAN payload (Only send what is editable)
      // We convert "5" to 5 (Integer) because APIs expect numbers for IDs.
      const payload = {
        ...
        // Ensure dropdowns send Numbers (or null if empty). 
        // Do NOT send empty strings "" as they cause 422 errors.
        ...
        // This list tells the server: "These are the ONLY bills in this shipment now."
        // The server will automatically remove any bills not in this list 
        // and add any new ones.
        custom_shipment_ids: attachedBills.map((b) => Number(b.id)),
      };

      console.log("Sending Payload:", payload); // Check console to see exactly what is sent

      // 2. Send Update
      await updateBillShipment(id, payload);
      toast.success("Shipment updated successfully!");
      
      // Optional: navigate("/billshipments");
    } catch (err) {
      console.error("Save error details:", err);
      
      // 3. Display the EXACT error message from the server
      const validationErrors = err?.response?.data?.errors;
      const serverMsg = err?.response?.data?.message;
      
      if (validationErrors) {
        // If the server says "The shipment number has already been taken", show that.
